...
from memory.memory_manager import load_memory, format_memory_for_prompt, build_system_context
    ...
    try:
        response = ollama.chat(
            ...
            messages=[
                ...
                {'role': 'user', 'content': f"Write Python code to accomplish this task:\n\n{description}"}
            ]
        )
...
def _detect_language(text: str) -> str:
    ...
    try:
        response = ollama.chat(
            model='llama3',
            messages=[
                {'role': 'user', 'content': f"What language is this text written in? Reply with ONLY the language name in English (e.g. Turkish, English, French).\n\nText: {text[:200]}"}
            ]
        )
...
class AgentExecutor:
    ...
    def execute(
        ...
        facts_cmds = build_system_context()
        context = f"{mem_str}\n{facts_cmds}".strip()
        ...
        while True:
            ...
            replan_attempts += 1
            plan = replan(goal, completed_steps, failed_step, failed_error, context=context)
